...
public class Game {
    ...
    private enum GeneralCommand{
        ...
        public int commandInvoked(String[] command){
            for (String denomination: possibleDenominations){  //go through the possible denominations which could invoke this generalCommand
                ...
                if (commandInvoked) return denominationAsStringArray.length;  //returns a positive integer, which signifies the next index to inspect in the inputted String[]. (for example if inputted line is 'inspect <item>', next index the commandHandler() should look at is 1, to determine the item.)
            }
    ...
    public void generatePlayingField(){
        ...
        rivendell.addNeighboringLocation(Direction.EAST, mistyMountains,false);
    ...
    public void look(Direction direction){
        ...
        if (currentLocation.getNeighboringLocation(direction).getAlreadyVisited()) System.out.println(currentLocation.getNeighboringLocation(direction).getName());  //The name of the location the player looks to, is only printed if he knows the location, i.e. has already visited the location.
